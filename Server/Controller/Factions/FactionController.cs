...
namespace Server.Controller.Factions
{
    ...
    public class FactionController : BaseClass
    {
          ...
          clientEventTriggerFunc, clientEventTriggerAllFunc)
        {
            ...
            EventHandlers[FactionEvents.CreateFactionRank] += new Action<Player, string, string, int>(OnCreateFactionRank);
            EventHandlers[FactionEvents.FactionRankRemove] += new Action<Player, string>(OnRemoveFactionRank);
            EventHandlers[FactionEvents.FactionRankRename] += new Action<Player, string, string>(OnRenameFactionRank);
        ...
        private async void OnRegisterFaction(string factionName, FactionRank[] ranks)
        {
            ...
            {
                Debug.WriteLine("Faction already exists. You can't register a faction twice.");
                return;
            }
        ...
        private async void OnRenameFaction([FromSource] Player player, string factionId, string newName)
        {
            ...
            {
                // Should never happen!
                player.TriggerEvent(FactionEvents.Error, new Error(ErrorTypes.FactionError, (int)ErrorCodes.NotFound));
                return;
            }
        ...
        {
            var rankToRemove = Context.FactionRanks.FirstOrDefault(r => r.Uuid == rankId);

            if (rankToRemove != null)
            {
                Context.FactionRanks.Remove(rankToRemove);
                await Context.SaveChangesAsync();
                player.TriggerEvent(FactionEvents.FactionRankRemoved);
            }
            else
            {
                new Error(player, ErrorTypes.FactionError, (int)ErrorCodes.RankNotFound);
            }
        }

        private async void OnRenameFactionRank([FromSource] Player player, string factionRankId, string newName)
        {
            var rankToRename = Context.FactionRanks.FirstOrDefault(r => r.Uuid == factionRankId);

            if (rankToRename != null)
            {
                rankToRename.Name = newName;
                await Context.SaveChangesAsync();
                player.TriggerEvent(FactionEvents.FactionRankRenamed);
            }
            else
            {
                new Error(player, ErrorTypes.FactionError, (int)ErrorCodes.RankNotFound);
        ...
        private async void OnEditFactionRankPermissions([FromSource] Player player, string factionRankId, RankPermissions rankPermissions)
        {
            var permissionsToEdit = Context.RankPermissions.FirstOrDefault(p => p.RankId == factionRankId);
            if (permissionsToEdit == null)
            {
                new Error(player, ErrorTypes.FactionError, (int)ErrorCodes.NotFound);
                return;
            }

            permissionsToEdit.AccessFinance = rankPermissions.AccessFinance;
            permissionsToEdit.AddRemoveMembers = rankPermissions.AddRemoveMembers;
            permissionsToEdit.AddRemoveRanks = rankPermissions.AddRemoveRanks;
            permissionsToEdit.AssignRanks = rankPermissions.AssignRanks;
            permissionsToEdit.CanRenameRanks = rankPermissions.CanRenameRanks;
